...
**Phone → desktop receive** (`webreceive.go`). Apple AirDrop / Android Quick Share **cannot be received** in this app — closed, app-only protocols (AWDL needs monitor-mode Wi-Fi + isn't on Windows; Quick Share is Ukey2-encrypted Nearby Connections). See `.claude/spikes/mobile-native-share-feasibility.md`; don't re-attempt them. The shipped path is a **token-gated local HTTPS upload** (TLS via `ephemeralCertificate`, self-signed → one-time browser warning; never plaintext). It is **always-on while the device is visible** (`ensureReceiving`, started at launch — not a manual toggle), so phones/LocalSend/etc. find krokodyl out of the box; `StartPhoneReceive` just returns the QR + URL. Files stream to the download dir via `safeUploadName` (path-stripped, traversal-rejected, deduped) and appear as completed transfers. **Two-token model** (the write credential never enters a URL): the QR URL carries a **single-use bootstrap token** that authorizes ONE page load — verified-before-consumed, then a `Secure`+`HttpOnly`+`SameSite=Strict` session cookie is set and we 303-redirect to a clean tokenless URL (so reloads work, but a replayed bootstrap URL from history is dead). The page embeds a separate **upload token** sent only in the `X-Krokodyl-Token` header (header-only — `/upload` rejects a URL token). Other guardrails that must stay: `ReadHeaderTimeout` (slow-loris), `MaxBytesReader` caps a request, `maxConcurrentUploads` bounds simultaneous streams (429 when saturated), `Referrer-Policy: no-referrer` + `Cache-Control: no-store` on the page, and `stopReceiving` closes every server on visibility-off / shutdown.
