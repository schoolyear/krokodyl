...
**LocalSend interop** (`localsend.go`). krokodyl speaks the open LocalSend v2 protocol (receive side) so the LocalSend app discovers it and sends to it — multicast announce on **224.0.0.167:53317** + the v2 HTTP API (info/register/prepare-upload/upload/cancel). Started with the same opt-in as the web receiver (`StartPhoneReceive`). Consent reuses the nearby-offer prompt: `prepare-upload` blocks on `localSendOffer` → emits `nearby:offer` → `RespondToNearbyOffer` routes to BOTH the croc nearby server and the LocalSend receiver (id matches one). Per-file tokens are issued only after accept, single-use, constant-time checked; uploads stream via the shared `saveUploadedFile`. Guardrails to keep: one pending offer at a time, session TTL, register-back is LAN-IP-only + semaphore-bounded (no SSRF/flood), copy session state under the mutex before streaming (concurrent uploads share the maps). **Multicast MUST use every real interface, not the OS default** (`runMulticast` + `multicastInterfaces`): on a multi-adapter Windows box (Hyper-V/WSL/NAT virtual switches) the OS-default multicast interface is frequently a virtual switch the phone can't hear, so we join the group on each real interface AND announce by setting `ipv4.PacketConn.SetMulticastInterface` per interface before each `WriteTo`. **Do NOT use `DialUDP`/bound-local-addr for egress** — that does not reliably set `IP_MULTICAST_IF` on Windows, so the announce silently leaves via a virtual switch (you'll still *hear* peers because the join side works, but they never see you — the exact one-way-discovery bug). **Port coexistence:** if TCP 53317 is busy (the official LocalSend app is running on the same box), bind an ephemeral port and announce it (`bound`) instead of failing — LocalSend peers use the announced port, so krokodyl and the LocalSend app run side by side.
